...
export default function NavBar() {
  ...
  const [estAgentVerifie, setEstAgentVerifie] = useState(false)
  ...
  useEffect(() => {
    if (user) {
      ...
      setEstAdmin(false)
      setEstAgentVerifie(false)
      setEstLocataire(false)
      setMessagesNonLus(0)
      setPhotoProfil('')
      setNomUtilisateur('')
    }
  ...
  async function verifierRole() {
    const { data: agent } = await supabase
      .from('agents')
      .select('id, role, photo_profil, nom, verifie')
      .eq('email', user?.email)
      .single()

    if (agent) {
      setEstAdmin(agent.role === 'admin')
      setEstAgentVerifie(agent.verifie === true)
  ...
  return (
    <>
      {/* NAVBAR HAUT — desktop uniquement */}
      ...
      {/* BARRE EN BAS — mobile uniquement */}
      <div className="fixed bottom-0 left-0 right-0 z-50 md:hidden bg-white border-t border-gray-200">
        <div className="grid grid-cols-4 h-16">
          ...
          <Link href="/" className={`flex flex-col items-center justify-center gap-1 transition ${actif('/') ? 'text-blue-800' : 'text-gray-400'}`}>
            <svg className="w-6 h-6" fill={actif('/') ? 'currentColor' : 'none'} stroke="currentColor" strokeWidth={1.8} viewBox="0 0 24 24">
          ...
          {/* PUBLIER — visible seulement pour les agents vérifiés */}
          {estAgentVerifie ? (
            ...
            <div className="flex flex-col items-center justify-center gap-1 opacity-30">
              <div className="w-12 h-12 rounded-full flex items-center justify-center bg-gray-300 -mt-4">
                <svg className="w-6 h-6 text-white" fill="none" stroke="currentColor" strokeWidth={2} viewBox="0 0 24 24">
                  <path strokeLinecap="round" strokeLinejoin="round" d="M12 4v16m8-8H4" />
                </svg>
              </div>
              <span className="text-xs font-medium mt-1 text-gray-400">Publier</span>
            </div>
          )}

          {/* MON COMPTE */}
          <Link href={user ? '/mon-espace' : '/auth'} className={`flex flex-col items-center justify-center gap-1 transition relative ${actif('/mon-espace') || actif('/dashboard') || actif('/locataire') || actif('/auth') ? 'text-blue-800' : 'text-gray-400'}`}>
            {photoProfil ? (
